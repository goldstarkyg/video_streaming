	...
	<script type="text/javascript">
	  ...
	  $('document').ready(function(){
	    ...
	    <?php if(session('note') != '' && session('note_type') != ''): ?>
	        var n = noty({text: '<?= str_replace("'", "\\'", session("note")) ?>', layout: 'top', type: '<?= session("note_type") ?>', template: '<div class="noty_message"><span class="noty_text"></span><div class="noty_close"></div></div>', closeWith: ['button'], timeout:1600 });
