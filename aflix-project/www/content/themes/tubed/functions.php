			...
			if($menu_item->type == 'videos'):
				$output .= '<li class="dropdown">';
					$output .= '<a href="/videos" class="dropdown-toggle">' . $menu_item->name . ' <span class="caret"></span></a>';
						...
						$output .= generate_video_post_menu(HelloVideo\Models\VideoCategory::orderBy('order', 'ASC')->get(), '/videos/category/');
...
if(!function_exists ( 'dynamic_styles' )):
	function dynamic_styles($theme_settings){
		$hex_color = HelloVideo\Libraries\ThemeHelper::getThemeSetting(@$theme_settings->color, '');
		if(empty($hex_color)){
			$hex_color = '#E52D27';
		}
