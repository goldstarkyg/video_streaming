...
<div class="col-md-10 col-md-offset-2 right-content-10 user">
	...
	<?php if(isset($type) && $type == 'profile') { ?>
		...
		<div id="user-badge">
			...
			<div class="label label-info"><?= ucfirst($user->role) ?> <?php if($user->contribute!=null){ echo " / Contribute"; } ?> User</div>
	...
	<?php  } else if(isset($type) && $type == 'edit'){ ?>
			...
			<?php if(($settings->free_registration && $settings->premium_upgrade) || (!$settings->free_registration)): ?>
			    <div class="well">
			        ...
			        <?php if($user->role == 'subscriber'): ?>
			            <div class="label label-success"><i class="fa fa-certificate"></i> <?= ucfirst($user->role) ?> <?php if($user->contribute!=""){ echo " / Contribute"; } ?> User</div>
			            <div class="clear"></div>
			        <?php elseif($user->role == 'registered'): ?>
			            <div class="label label-warning"><i class="fa fa-user"></i> <?= ucfirst($user->role) ?> <?php if($user->contribute!=""){ echo " / Contribute"; } ?> User</div>
			            <div class="clear"></div>
			        ...
			        <?php if($settings->free_registration && $settings->premium_upgrade): ?>
			            <a class="btn btn-primary" href="<?= ($settings->enable_https) ? secure_url('/') : URL::to('user') ?><?= '/' . $user->username; ?>/renew_subscription" style="margin-top:10px;"><i class="fa fa-certificate"></i> Upgrade to Premium Subscription</a>
			        <?php else: ?>
	...
	<?php } else if(isset($type) && $type == 'billing'){ ?>

		<?php include('partials/user-billing.php'); ?>

	<?php } else if(isset($type) && $type == 'update_credit_card'){ ?>

		<?php include('partials/user-update-billing.php'); ?>

	<?php }else if(isset($type) && $type == 'renew_subscription'){ ?>

		<?php include('partials/renew-subscription.php'); ?>

	<?php } else if(isset($type) && $type == 'upgrade_subscription'){ ?>
