...
<div class="col-md-10 col-md-offset-2 right-content-10">
    ...
    <?php if (Session::has('notification')): ?>
        <span class="notification"><?= session('notification') ?></span>
    <?php endif; ?>
    ...
    <form method="POST" action="<?= ($settings->enable_https) ? secure_url('password/reset') : URL::to('password/reset') ?>" accept-charset="UTF-8" class="form-signin">
        <?php if (Session::has('error')): ?>
          <span class="error"><?= trans(session('reason')) ?></span>
        ...
        <input name="email" type="text" class="form-control" placeholder="Email Address"  value="<?php if(Auth::check()){ echo Auth::user()->email; } ?>">
